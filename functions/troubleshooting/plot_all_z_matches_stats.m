function all_stats = plot_all_z_matches_stats(secs, start, finish)
% Plot correspondence stats dataset for distributions

% Inputs:
%   secs: cell array of sections with z matches & alignment
%
% Outputs:
...
name = sprintf('%s plot_all_z_matches_stats', secs{start}.wafer);
figure('name', name);
subplot(2, 1, 1);
...
xlabel('Section');
ylabel('Azimuth angle B-to-A (rad) [polar ignored]');
grid on
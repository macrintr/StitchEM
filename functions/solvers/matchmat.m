...
cum_num_tiles = cumsum(num_tiles) - num_tiles; % num_tiles(1) doesn't catch all cases
...
for s = 1:num_secs
    tile_idx = zeros(1, max(tile_nums{s}));
    % for t = min(tile_nums{s}):max(tile_nums{s})
    for t = tile_nums{s}'
        idx = find(tile_nums{s} == t, 1);
        if ~isempty(idx)
            tile_idx(t) = idx;
            % tile_idx(t) = idx; % Will add length if t > length(tile_idx) !
        end
    end
